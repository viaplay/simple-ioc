...
module.exports = function( ioc, trueCallback ) {
	var pub = {},
		resolver = require( './containerHelpers/resolver.js' )( trueCallback ),
	...
	pub.renameRegistered = function( from, to ) {
		return pub
			.registerInjectable( to, store.getInjectable( from ).fn )
			.removeRegistered( from );
	};
	...
	pub.resolveAllAndInject = function( fn ) {
		if( !ts )
			ts = Date.now();
		var nextResolvable = store.getAllResolvable().filter( function( name ) {
			if( currentResolvning[ name ] )
				return false;
			else {
				currentResolvning[ name ] = true;
				return true;
			}
		} );
				...
				if( shouldResolveAllAndInject )
					nextResolvable.forEach( function( resolvable ) {
						...
						pub.resolve( resolvable, function( err, instance ) {
							if( trueCallback && err )
								log.fatal( 'Component "' + resolvable + '" returned an error', err );
